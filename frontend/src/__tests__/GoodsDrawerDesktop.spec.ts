...
describe('GoodsDrawer desktop detail panel', () => {
  ...
  it('shows the core asset fields and image rail on desktop', async () => {
    ...
    expect(wrapper.findAll('.desktop-chip-row .desktop-chip')).toHaveLength(3)
    expect(wrapper.find('.desktop-chip-row .is-theme').exists()).toBe(true)
    expect(wrapper.find('.desktop-summary-row .desktop-theme-chip').exists()).toBe(false)
