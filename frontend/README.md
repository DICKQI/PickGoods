...
社团账号使用 `/club/goods`、`/club/popularity` 和 `/club/profile` 管理公开目录、用户人气反馈与社团资料。目录支持状态筛选、关键词搜索、批量上下架、拖拽调整公开顺序和草稿定时上架；定时上架时间按北京时间输入，后端以 UTC 保存。新增/编辑谷子时移动端隐藏底部导航，避免长表单被遮挡。
