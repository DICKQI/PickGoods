...
- **URL**：`GET /api/themes/{id}/template/`（获取）
- **URL**：`POST /api/themes/{id}/template/`（创建/更新）
- **说明**：管理主题的默认模板参数，包括默认排布位置（characters_x / characters_y）、默认角色筛选列表等。用于在创作页面时预填默认布局。
...
```json
{
  "template": {
    ...
    "theme_id": 1,
    "characters": [
      { "id": 5, "name": "流萤", "ip": { "id": 1, "name": "崩坏：星穹铁道", "subject_type": 4 }, "avatar": null, "gender": "female" }
    ],
    "characters_x": 100,
    "characters_y": 200,
    "created_at": "2024-09-21T10:00:00Z",
  ...
  "images": [
    { "id": 1, "image": "http://example.com/media/themes/extra/xxx.jpg", "label": "海报" }
  ]
}
```
...
| 字段名          | 类型         | 必填 | 说明                                      |
| --------------- | ------------ | ---- | ----------------------------------------- |
| `characters`    | array[int]   | 否   | 角色 ID 列表，用于默认排布                |
| `characters_x`  | float        | 否   | 角色区域横向起始位置                      |
| `characters_y`  | float        | 否   | 角色区域纵向起始位置                      |
| `width`         | float        | 否   | 页面宽度                                  |
| `height`        | float        | 否   | 页面高度                                  |
...
- **URL**：`POST /api/themes/{id}/copy-images-from-goods/`
- **说明**：将指定谷子的主图和附加图片复制到主题的图片池中。该谷子必须属于该主题。常用于在创作页面时快速将谷子图片用作主题背景/素材。
...
| 字段名         | 类型   | 说明                         |
| -------------- | ------ | ---------------------------- |
| `copied_count` | int    | 成功复制的图片数量           |
| （其余字段）   |        | 主题详情（同 5.4.2）         |

##### 约束

- 只能复制属于该主题的谷子图片（`goods.theme_id == theme.id`）
- 不会覆盖已存在的主题图片，始终新增
...
### 8.3 搜索 IP 作品列表（两步搜索第一步）

- **URL**：`POST /api/bgm/search-subjects/`
- **说明**：
  - 通过关键字搜索 BGM 上相关的 IP 作品列表，获取作品 ID 和基本信息。
  - 作为两步搜索的第一步，搜索结果中的作品 ID 可用于第二步（8.4）获取角色。
  - **不会**写入本地数据库，仅用于前端展示候选作品列表。
  - 与 8.1 接口的区别：8.1 直接搜索作品并一起返回角色列表；本接口只搜索作品列表（不含角色），适用于需要先让用户选择作品的场景。
  ...
  "subjects": [
    {
      ...
      "type": 4,
      "type_name": "游戏",
      "image": "https://lain.bgm.tv/pic/cover/l/xx/xx/12345.jpg"
    },
    {
      "id": 67890,
      "name": "崩坏3",
      ...
      "type": 4,
      "type_name": "游戏",
      "image": "https://lain.bgm.tv/pic/cover/l/yy/yy/67890.jpg"
    }
  ]
}
```
...
| 字段名     | 类型           | 说明                                      |
| ---------- | -------------- | ----------------------------------------- |
| `subjects` | array[object]  | 作品列表                                  |
| `id`       | integer        | BGM 作品 ID，用于第二步调用                |
| `name`     | string         | 作品原名                                  |
| `name_cn`  | string         | 作品中文名，可能为空                      |
| `type`     | integer        | 作品类型代码：`1`=书籍, `2`=动画, `3`=音乐, `4`=游戏, `6`=三次元/特摄 |
| `type_name`| string         | 作品类型名称，如：`"游戏"`、`"动画"`      |
| `image`    | string         | 作品封面图 URL，可能为空                  |

---

### 8.4 根据 BGM 作品 ID 获取角色列表（两步搜索第二步）

- **URL**：`POST /api/bgm/get-characters-by-id/`
- **说明**：
  - 根据在 8.3 接口中选定的 BGM 作品 ID，获取该作品下的全部角色信息。
  - 作为两步搜索的第二步，返回的角色列表可供用户勾选导入（使用 8.2 接口）。
  - **不会**写入本地数据库。
...
| 字段名       | 类型    | 必填 | 说明              |
| ------------ | ------- | ---- | ----------------- |
| `subject_id` | integer | 是   | BGM 作品 ID       |
...
| 字段名         | 类型           | 说明                                      |
| -------------- | -------------- | ----------------------------------------- |
| `subject_id`   | integer        | BGM 作品 ID                               |
| `subject_name` | string         | 作品名称                                  |
| `characters`   | array[object]  | 角色对象数组，字段同 8.1 的 `characters`  |
| `name`         | string         | 角色名（已解码 HTML 实体）                |
| `relation`     | string         | 角色与作品关系：如「主角」「配角」「客串」 |
| `avatar`       | string         | 角色头像 URL                              |

> 注意：与 8.1 接口返回的角色列表格式相同，角色按关系排序（主角在前、配角其后、客串/其他在最后）。头像 URL 可在后续创建角色时（8.2 接口）使用。

#### 响应体（未找到示例）

```json
{
  "detail": "未找到ID为 99999 的作品"
}
```
...
1. 用户在前端输入关键词并点击「搜索 BGM」：
   - 调用 `POST /api/bgm/search-subjects/`（8.3），获取候选作品列表（包含作品ID）。
2. 用户从候选作品列表中选择目标作品：
   - 调用 `POST /api/bgm/get-characters-by-id/`（8.4），传入选定的 `subject_id` 获取角色列表。
3. 前端展示从 BGM 返回的角色列表，用户勾选需要导入的角色：
   - 将勾选结果映射为包含 `ip_name`、`character_name`、`avatar`（可选）的数组。
   - **推荐**：将 BGM 返回的 `avatar` URL 也一起传递。
4. 用户点击「确认导入」：
   - 调用 `POST /api/bgm/create-characters/`（8.2），传递角色信息（包含头像URL）；
   - 导入完成后，前端可以根据返回的 `ip_id` / `character_id` 刷新本地 `IP` / `Character` 列表或直接追加。

> 两种方式的对比：
> - **方式一（一步搜索）**：接口少、流程短，适合 IP 名称明确且搜索结果精确的场景。
> - **方式二（两步搜索）**：灵活性更高，适合 IP 名称可能模糊匹配到多个作品时，让用户先确认具体是哪个作品再拉取角色。
...
> **补充说明**：标准的 `GET /api/showcases/` 端点（由 DRF Router 自动注册）也可使用，返回当前用户自己的展柜与公开展柜（需要登录）。以下推荐使用语义更明确的公共/私有拆分接口。
...
1. **级联删除**：删除展柜时，会同时删除该展柜下的所有谷子关联（`ShowcaseGoods`）
...
假设展柜中的顺序为：A(order=10) -> B(order=20) -> C(order=30)。
...
3. **管理展柜**
   - 查看展柜详情：`GET /api/showcases/{id}/`（包含所有谷子）
...
|------|----------------|---------|
| 数据范围 | 所有谷子 | 用户选择的谷子 |
| 组织方式 | 按 IP/角色/品类等维度筛选 | 按展柜组织 |
| 用途 | 检索和管理所有资产 | 自定义展示特定谷子 |
| 排序 | 全局排序 | 展柜内排序 |

#### 9.4.3 典型使用场景

- **主题展柜**：创建一个"星铁主题展柜"，将相关谷子添加到展柜
...
Admin 后台管理接口用于管理员进行用户和角色的管理。所有接口需要登录且仅限 `Admin` 角色访问。

> **鉴权说明**：本章节所有接口均需要登录，并在请求头携带 `Authorization: Bearer <access_token>`。  
> 调用者 `role.name` 必须为 `"admin"`，否则返回 `403 Forbidden`。***
...
| `id`         | int     | 用户 ID                          |
| `username`   | string  | 用户名                           |
| `role`       | object  | 角色对象，包含 `id`、`name`、`created_at` |
| `is_active`  | boolean | 是否启用，`false` 表示已停用     |
...
| `password` | string | 是   | 登录密码，最少 6 位      |
| `role_id`  | int    | 是   | 所属角色 ID              |

##### 响应

返回创建后的用户详情（同 10.1.2），状态码 `201 Created`。

---

#### 10.1.4 更新用户

- **URL**：`PUT /api/admin/users/{id}/`（完整更新）
- **URL**：`PATCH /api/admin/users/{id}/`（部分更新）
- **说明**：更新用户信息，可修改角色、启用状态和密码。不提供DELETE：请使用 PATCH 将 `is_active` 设为 `false` 停用账号。
...
| ---------- | ------- | ---- | --------------------------------------- |
| `role_id`  | int     | 否   | 角色 ID                                 |
| `is_active`| boolean | 否   | 是否启用，`false` 表示停用该账号        |
| `password` | string  | 否   | 新密码（最轻 6 位）。不传则不修改密码   |
...
- **说明**：获取所有角色的列表，供后台分配用户角色时下拉使用。只读接口。
...
BGM 自动同步功能用于定时从 Bangumi API 拉取角色列表，与本地 IP/Character 数据自动同步，支持定时自动执行和手动触发。

> **鉴权说明**：本章节所有接口均需要登录，且仅限 `Admin` 角色访问。

#### 10.3.1 获取/更新同步配置

- **URL**：`GET /api/admin/bgm-sync/settings/`（获取配置）
- **URL**：`PATCH /api/admin/bgm-sync/settings/`（部分更新）
- **URL**：`PUT /api/admin/bgm-sync/settings/`（完整更新）
- **说明**：读取或更新全局 BGM 自动同步配置（单例模式）。
...
```json
{
  ...
  "enabled": true,
  "max_ips_per_run": 5,
  "sync_subject_type": true
}
```

| 字段名              | 类型    | 必填 | 说明                                                       |
| ------------------- | ------- | ---- | ---------------------------------------------------------- |
| `cron_expression`   | string  | 否   | Cron 表达式，定义自动执行时间，默认 `0 2 * * *`（每天凌晨2点） |
| `enabled`           | boolean | 否   | 是否启用自动同步                                           |
| `max_ips_per_run`   | int     | 否   | 每次运行最多同步的 IP 数量，默认 5                         |
| `sync_subject_type` | boolean | 否   | 是否同步更新 IP 的 `subject_type`                          |
...
- **URL**：`POST /api/admin/bgm-sync/run-now/`
- **说明**：立即在后台线程启动一次 BGM 同步。若已有同步任务正在执行，返回 409 Conflict。
- **请求体**：无
...
```json
{
  ...
  "status": "running",
  "trigger": "manual",
  "triggered_by": { "id": 1, "username": "admin" },
  "started_at": "2024-09-21T10:00:00Z",
  "summary": { "total_ips": 5, "success": 0, "no_change": 0, "skipped": 0, "errors": 0 }
}
```

#### 10.3.3 同步任务历史

- **URL**：`GET /api/admin/bgm-sync/jobs/`
- **说明**：分页查看自动/手动同步任务历史。
...
| `status__in`  | string | 多状态过滤，逗号分隔                                    |
| `trigger`     | string | 按触发方式过滤：`schedule` / `manual`                    |
| `started_at__date__gte` | date | 开始时间下界（含）                                |
| `started_at__date__lte` | date | 开始时间上界（含）                                |
  ...
  "results": [
    {
      ...
      "triggered_by": { "id": 1, "username": "admin" },
      "started_at": "2024-09-21T10:00:00Z",
      "finished_at": "2024-09-21T10:05:00Z",
      "summary": { "total_ips": 5, "success": 3, "no_change": 1, "skipped": 0, "errors": 1 }
    }
  ]
}
```
...
- **URL**：`GET /api/admin/bgm-sync/jobs/{id}/`
- **说明**：获取单个同步任务的详细信息，包含完整的 summary。

#### 10.3.5 获取同步任务 IP 明细

- **URL**：`GET /api/admin/bgm-sync/jobs/{id}/items/`
- **说明**：分页获取某次同步任务中各 IP 的同步结果明细。
...
| `page`             | int    | 分页页码                                          |
| `page_size`        | int    | 每页数量，默认 20，最大 100                       |
| `status`           | string | 按明细状态过滤：`success` / `no_change` / `skipped_unbound` / `error` |
| `ip_name_snapshot` | string | IP 名称模糊搜索                                     |
  ...
  "results": [
    {
      "id": 1,
      "job_id": "job-uuid-...",
      "ip_name_snapshot": "崩坏：星穹铁道",
      "status": "success",
      "summary": { "created": 2, "matched": 10, "errors": 0 },
      "error_message": null,
      "processed_at": "2024-09-21T10:01:00Z"
    }
  ]
}
```
...
项目集成 `drf-spectacular`，生成 OpenAPI 3.0 规范的 Schema 文件，并提供了 Swagger UI 和 ReDoc 两种交互式文档页面。无需登录即可访问。

| 方法 | URL                         | 说明                                            |
| ---- | --------------------------- | ----------------------------------------------- |
| GET  | `/api/schema/`              | OpenAPI 3.0 Schema（YAML 格式），供代码生成工具使用 |
| GET  | `/api/schema/swagger-ui/`   | Swagger UI 交互式文档页面，可直接在线测试 API     |
| GET  | `/api/schema/redoc/`        | ReDoc 文档页面，适合查看和分享                    |

> 开发环境可通过 Swagger UI 页面直接调试所有接口（需先通过登录接口获取 Token，在页面右上角 Authorize 按钮中填入 `Bearer <token>`）。

---

## 十二、OCR 识别接口

用于从谷子订单截图（例如淘宝订单截图）中 OCR 解析出谷子名称、价格、数量、购入日期等信息，并匹配本地数据库中的 IP、角色、品类等元数据。

> **鉴权说明**：需要登录，并在请求头携带 `Authorization: Bearer <access_token>`。

### 12.1 订单截图 OCR 识别

- **URL**：`POST /api/ocr/recognize/`
- **请求方式**：`multipart/form-data`
- **说明**：上传订单截图（淘宝等），自动 OCR 识别并结构化返回谷子信息，同时基于本地数据库匹配 IP、角色、品类建议。

#### 请求体（multipart/form-data）

| 字段名                | 类型   | 必填 | 说明                                                    |
| --------------------- | ------ | ---- | ------------------------------------------------------- |
| `image`               | file   | 是   | 图片文件，最大 10MB，支持 JPG/PNG 等常见格式            |
| `confidence_threshold`| float  | 否   | 元数据匹配置信度阈值（0~1），默认 0.0，高于阈值才保留   |
...
```json
{
  ...
  "is_official": true,
  "shop_name": "哔哩哔哩会员购",
  "raw_text": "商品名称：崩铁 流萤 立牌\n价格：89.00\n数量：1",
  "suggestions": {
    "ip": {"id": 1, "name": "崩坏：星穹铁道", "confidence": 0.95},
    "characters": [
      {"id": 5, "name": "流萤", "confidence": 0.92}
    ],
    "category": {"id": 2, "name": "立牌", "confidence": 0.85}
  },
  "items": [
    {
      "name": "崩铁 流萤 立牌",
      ...
      "is_official": true,
      "shop_name": "哔哩哔哩会员购",
      "suggestions": { "ip": {...}, "characters": [...], "category": {...} }
    }
  ]
}
```
...
| 字段名         | 类型            | 说明                                                               |
| -------------- | --------------- | ------------------------------------------------------------------ |
| `name`         | string          | 谷子名称（从截图 OCR 解析）                                        |
| `price`        | string          | 价格                                                               |
| `quantity`     | int             | 数量                                                               |
| `purchase_date`| string          | 购入日期（YYYY-MM-DD）                                             |
| `is_official`  | bool            | 是否官谷（后端根据关键词规则判定）                                 |
| `shop_name`    | string          | 店铺名                                                             |
| `raw_text`     | string          | OCR 识别的全部文本                                                 |
| `suggestions`  | object          | **元数据匹配建议**：基于识别文本与本地 IP/角色/品类库进行模糊匹配，用于前端预填表单 |
| `ip`           | object/null     | 匹配到的 IP 对象：`{id, name, confidence}`                         |
| `characters`   | array[object]   | 匹配到的角色列表：`[{id, name, confidence}]`                       |
| `category`     | object/null     | 匹配到的品类对象：`{id, name, confidence}`                         |
| `items`        | array[object]   | 完整解析条目列表（当截图包含多个商品时），每项结构与外层一致       |

#### 错误响应

- `400 Bad Request`：未提供图片 / 图片超过 10MB / 无法从图片中识别文字 / 无法解析商品条目
- `500 Internal Server Error`：OCR 模型识别失败

#### 使用场景

- **录入辅助**：用户拍下淘宝订单截图，OCR 自动填充谷子信息，提高录入效率
- **预填表单**：匹配到的 IP、角色、品类建议可直接用于新建谷子接口

> **缓存说明**：OCR 推理结果基于图片 SHA256 缓存 5 分钟，同一张图片在缓存时间内直接返回结果。

---

## 十三、谷子手帐 API

手帐功能用于创建**排版式图文页面**，类似「手帐/笔记」形式展示谷子收藏。用户可创建多个手帐本（JournalBook），每个本中包含若干页面（JournalPage），每个页面的编辑历史通过版本（JournalPageVersion）管理。

> **鉴权说明**：本章节所有接口均需要登录，并在请求头携带 `Authorization: Bearer <access_token>`。  
> 普通用户只可见和操作自己的手帐；管理员可查看全部。
...
| `id`          | UUID (PK)             | 手帐本唯一 ID                                      |
| `name`        | Char(200)             | 手帐名称                                           |
| `description` | Text (可空)           | 手帐描述                                           |
| `cover_image` | Image(URL, 可空)      | 封面图片                                           |
| `order`       | BigInteger            | 排序值，值越小越靠前，默认 0                       |
| `created_at`  | DateTime              | 创建时间                                           |
| `updated_at`  | DateTime              | 更新时间                                           |
...
| ----------------- | --------------------- | -------------------------------------------------- |
| `id`              | UUID (PK)             | 页面唯一 ID                                        |
| `book`            | FK -> `JournalBook`   | 所属手帐本（CASCADE）                              |
| `title`           | Char(200, 可空)       | 页面标题，自动生成为"第 X 页"                      |
| `page_no`         | Integer               | 页码                                               |
| `content`         | JSON (可空)           | 页面内容（富文本/组件布局数据）                    |
| `width`           | Float，默认 800       | 页面宽度                                           |
| `height`          | Float，默认 600       | 页面高度                                           |
| `background`      | Char(50, 可空)        | 背景色/背景图片                                    |
| `background_style`| Char(50, 可空)        | 背景样式（cover/contain/repeat）                   |
| `preview_image`   | Image(URL, 可空)      | 页面预览图                                         |
| `revision`        | Integer，默认 1       | 修订号，用于冲突检测                               |
| `created_at`      | DateTime              | 创建时间                                           |
| `updated_at`      | DateTime              | 更新时间                                           |

#### `JournalPageVersion` 页面版本

| 字段名          | 类型                    | 说明                                               |
| --------------- | ----------------------- | -------------------------------------------------- |
| `id`            | UUID (PK)               | 版本唯一 ID                                        |
| `page`          | FK -> `JournalPage`     | 所属页面（CASCADE）                                |
| `version_no`    | Integer                 | 版本号，从 1 开始递增                              |
| `content`       | JSON (可空)             | 该版本的内容快照                                   |
| `preview_image` | Image(URL, 可空)        | 该版本的预览图快照                                 |
| `created_at`    | DateTime                | 创建时间                                           |
...
- **URL**：`GET /api/journals/`
- **说明**：分页获取手帐本列表，按排序值和更新时间倒序。
...
- **URL**：`GET /api/journals/{id}/`
- **说明**：获取单个手帐本详情，包含所有页面摘要信息。
...
- **URL**：`POST /api/journals/`
- **说明**：创建新谷子手帐本，创建后会自动生成一页空白页。
...
| 字段名        | 类型   | 必填 | 说明                           |
| ------------- | ------ | ---- | ------------------------------ |
| `name`        | string | 是   | 手帐本名称，最大长度200字符    |
| `description` | string | 否   | 手帐本描述                     |
| `cover_image` | file   | 否   | 封面图片，multipart/form-data  |

##### 响应

返回创建后的手帐本详情（同 13.2.2），状态码 `201 Created`。

#### 13.2.4 更新手帐本

- **URL**：`PUT /api/journals/{id}/`（完整更新）
- **URL**：`PATCH /api/journals/{id}/`（部分更新）
- **说明**：更新手帐本信息。
...
| 字段名        | 类型   | 必填 | 说明                           |
| ------------- | ------ | ---- | ------------------------------ |
| `name`        | string | 否   | 手帐本名称，PUT 必填          |
| `description` | string | 否   | 手帐本描述                     |
| `cover_image` | file   | 否   | 封面图片，multipart/form-data  |

#### 13.2.5 删除手帐本

- **URL**：`DELETE /api/journals/{id}/`
- **说明**：删除手帐本，会同时级联删除所有页面和版本历史。
- **响应**：`204 No Content`
...
| 参数名   | 类型   | 说明                                               |
| -------- | ------ | -------------------------------------------------- |
| `fields` | string | 设为 `summary` 时返回精简摘要；不传时返回完整内容 |
...
- **URL**：`POST /api/journals/{id}/pages/`
- **说明**：在手帐本下创建新页面。自动分配页码（最后页 +1），标题自动生成为"第 X 页"。
...
返回创建的完整页面对象，状态码 `201 Created`。

#### 13.3.3 重新排序页面

- **URL**：`POST /api/journals/{id}/pages/reorder/`
- **说明**：批量重新编排所有页面顺序。需提供手帐本下所有页面的完整 ID 列表，按新顺序排列。系统会重新分配页码（1、2、3...），同时自动更新"第 X 页"格式的标题。

##### 请求体（JSON）

```json
{
  "page_ids": ["id3", "id1", "id2"]
}
```
...
- **URL**：`GET /api/journal-pages/{id}/`
- **说明**：获取单个页面的完整信息（含内容 JSON）。

#### 13.4.2 更新页面

- **URL**：`PUT /api/journal-pages/{id}/`（完整更新）
- **URL**：`PATCH /api/journal-pages/{id}/`（部分更新）
...
| ------------------ | ------- | ---- | ---------------------------------------------------------- |
| `title`            | string  | 否   | 页面标题                                                   |
| `content`          | object  | 否   | 页面内容 JSON                                              |
| `width`            | float   | 否   | 页面宽度                                                   |
| `height`           | float   | 否   | 页面高度                                                   |
| `background`       | string  | 否   | 背景色/背景图片 URL                                        |
| `background_style` | string  | 否   | 背景样式：`cover` / `contain` / `repeat`                   |
| `revision`         | integer | 是   | **修订号**：必须传入当前修订号，用于冲突检测。若服务端已更新会返回 409 |
| `create_version`   | boolean | 否   | 是否自动创建版本快照，默认 `true`                          |

##### 并发冲突处理

当 `revision` 与服务端当前版本不一致时，返回 **409 Conflict**：
...
- **URL**：`DELETE /api/journal-pages/{id}/`
- **说明**：删除指定页面，会同时删除所有版本历史。
- **响应**：`204 No Content`

#### 13.4.4 上传页面预览图

- **URL**：`POST /api/journal-pages/{id}/upload-preview/`
- **请求方式**：`multipart/form-data`
- **字段**：`preview_image`（文件，必填）
- **说明**：上传或更新页面预览图，后台自动压缩至约 500KB 以下。

#### 13.4.5 复制页面

- **URL**：`POST /api/journal-pages/{id}/duplicate/`
- **说明**：复制当前页面为手帐本的新页面（含内容副本），自动分配下一个页码。
- **响应**：返回新创建的页面对象，状态码 `201 Created`。

### 13.5 页面版本管理

#### 13.5.1 获取版本历史

- **URL**：`GET /api/journal-pages/{id}/versions/`
- **说明**：分页获取指定页面的版本历史（按版本号倒序，最新在前）。
...
| ----------- | ---- | -------------------------------------- |
| `page`      | int  | 分页页码                               |
| `page_size` | int  | 每页数量，默认 50，最大 50             |
...
- **URL**：`GET /api/journal-page-versions/{id}/`
- **说明**：获取版本完整信息（含内容 JSON 快照）。

#### 13.5.3 删除版本

- **URL**：`DELETE /api/journal-page-versions/{id}/`
- **说明**：删除指定版本。
- **响应**：`204 No Content`

#### 13.5.4 恢复版本

- **URL**：`POST /api/journal-page-versions/{id}/restore/`
- **说明**：将页面内容恢复为指定版本的内容和预览图快照，修订号 +1，同时创建新版本。
- **响应**：返回恢复后的页面详情。

> **自动版本清理**：每个页面最多保留 50 个版本，超过时自动删除最早版本。
...
- **说明**：
  - 上传谷子图片，后端基于图像分析自动分类品类形状。
  - 支持返回 `round`（圆形，如吧唧/徽章）或 `rectangle`（矩形，如小卡/拍立得/色纸）两种形状类型。
  - 根据形状类型给出建议的匹配品类列表，用于快速录入品类。
  - **不会**写入数据库，仅用于前端预填建议。
- **请求方式**：`multipart/form-data`
- **鉴权**：需要登录
...
| 字段名       | 类型            | 说明                                                         |
| ------------ | --------------- | ------------------------------------------------------------ |
| `shape_type` | string          | 识别的形状类型：`round`（圆形）或 `rectangle`（矩形）        |
| `confidence` | float           | 置信度（0~1），值越高表示识别越确信                           |
| `suggestions`| array[object]   | **建议的品类列表**：根据形状类型按匹配度排序，每项包含 `id`、`name`、`path_name`、`shape_type` |

#### 错误响应

- `422 Unprocessable Entity`：未能识别谷子形状，响应：
  ```json
  { "shape_type": null, "confidence": 0.0, "suggestions": [], "detail": "未能识别图片中谷子的品类形状" }
  ```

#### 使用场景

- **品类快速选择**：新建谷子时，先拍照分类形状，然后从建议品类列表中选择，替代手动下拉选择。
