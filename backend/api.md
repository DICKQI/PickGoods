...
- **说明**：
  ...
  - 通过元数据与主图指纹融合评分，将相似的谷子整理在一起。
  - 复用“拍图识谷子”的 DINOv2 384 维向量和 pHash，不保存查询图片。
  - 提供更好的浏览体验，帮助用户发现相关的谷子。
  - 响应格式与标准列表接口（4.1）完全相同，支持分页和所有过滤器。

#### 相似度算法说明

默认总分 100 分，其中元数据占 75 分、主图指纹占 25 分。图片权重可通过
`GOODS_SIMILAR_IMAGE_WEIGHT` 调整（范围 0～100）。

| 维度 | 权重 | 说明 |
| ---- | ---- | ---- |
| 主图指纹 | 25分 | 384维余弦相似度映射到0～25分；pHash 距离不超过同款阈值时按满分计 |
| IP匹配 | 22.5分 | 相同IP得满分，相同作品类型得约7.425分 |
| 角色重叠 | 17.25分 | `共享角色数 / 总角色数 × 17.25` |
| 品类层级 | 13.5分 | 相同叶子品类13.5分，相同父级8.1分，相同祖父级4.05分 |
| 主题匹配 | 11.25分 | 相同主题得11.25分 |
| 价格区间 | 6分 | 价格差异≤10%得6分，≤25%得3.75分，≤50%得2.25分 |
| 入手日期 | 4.5分 | 相差≤30天得4.5分，≤90天得约3.195分，≤180天得约1.935分 |

一方或双方没有可用主图指纹时，图片维度不计分，元数据按原始 100 分范围重新归一化，
避免仅因缺少指纹而被额外降权。

**分组策略**：
1. 从不同IP中选择种子谷子（确保多样性）
2. 为每个种子找到融合分≥40分的谷子组成分组
3. pHash 同款命中的候选不受40分限制，并优先进入分组
4. 按种子IP轮询交错不同分组
...
- 完整排序结果最长缓存5分钟，排序时间窗口为2分钟；相邻页复用同一排序
- 18条及以下也使用相同的元数据与主图融合算法
...
- **URL**：`GET /api/themes/{id}/template/`（读取）
- **URL**：`POST /api/themes/{id}/template/`（创建/更新）
- **说明**：
  - 为主题配置新建谷子时的默认值模板（IP、角色、名称、入手日期、是否官谷、备注）。
  - 该模板与主题为一对一关系，用于批量录入同一主题的谷子时自动填充常用字段。
  - `GET` 返回当前模板数据（如未设置则 `template` 字段为 `null`）及该主题的所有图片。
  - `POST` 创建或更新模板（upsert 语义）。
...
```json
{
  "template": {
    ...
    "theme": 1,
    "name": "默认商品名",
    "ip": { "id": 1, "name": "原神", "subject_type": 4 },
    "ip_id": 1,
    "characters": [ { "id": 42, "name": "荧", "ip": { "id": 1, "name": "原神", "subject_type": 4 }, "avatar": null, "gender": "female" } ],
    "character_ids": [42],
    "purchase_date": null,
    "is_official": true,
    "notes": null,
  ...
  "images": [
    { "id": 5, "image": "http://example.com/media/themes/extra/abc.jpg", "label": "主图" }
  ]
}
```

`template` 为 `null` 时表示尚未设置模板。
...
| 字段名 | 类型 | 必填 | 说明 |
| ------ | ---- | ---- | ---- |
| `ip_id` | integer | 是 | 默认 IP 作品 ID |
| `name` | string | 是 | 默认商品名称 |
| `character_ids` | array[int] | 否 | 默认关联角色 ID 数组 |
| `purchase_date` | string(date) | 否 | 默认入手日期 |
| `is_official` | boolean | 否 | 是否官谷 |
| `notes` | string | 否 | 默认备注 |

##### POST 响应

返回更新后的模板数据（同 GET 响应中的 `template` 对象）。

> 注意：如果同时提供了 `ip_id` 和 `character_ids`，后端会校验所有角色是否属于该 IP，不匹配则返回验证错误。
...
- **说明**：将指定谷子的主图和附加图片复制到主题的图片池中，方便将某个谷子的素材图复用到主题展示。
...
| 字段名 | 类型 | 必填 | 说明 |
| ------ | ---- | ---- | ---- |
| `goods_id` | UUID(string) | 是 | 要复制图片的谷子 ID |
...
```json
{
  ...
  "created_at": "2024-06-01T00:00:00Z",
  "images": [
    { "id": 1, "image": "http://example.com/media/themes/extra/xxx.jpg", "label": "主图" },
    { "id": 2, "image": "http://example.com/media/themes/extra/yyy.jpg", "label": "附件图片" }
  ],
  "template": null
}
```
...
**字段说明**：
- `copied_count`：成功复制的图片数量
- 其余字段为更新后的主题详情

##### 约束说明
- 谷子必须属于该主题（`goods.theme_id == theme.id`），否则返回 400
- 谷子不存在或不属于主题用户时返回 404
- 主图标签固定为 `"主图"`，附加图片保留原标签（未设置时使用 `"附件图片"`）
...
BGM 同步为管理员提供自动/手动从 Bangumi 拉取并同步角色数据的能力。所有接口仅限 `Admin` 角色访问。

#### 10.3.1 BGM 同步配置

- **URL**：`GET /api/admin/bgm-sync/settings/`（读取）
- **URL**：`PUT /api/admin/bgm-sync/settings/`（完整更新）
- **URL**：`PATCH /api/admin/bgm-sync/settings/`（部分更新）
- **说明**：读取或更新 BGM 自动同步的全局配置（单例模式）。所有字段均为可选。
...
| `frequency` | string | 同步频率：`"daily"` / `"every_3_days"` / `"weekly"` |
| `last_run_at` | datetime | 上次同步时间（`null` 表示从未运行） |
| `next_run_at` | datetime | 下次计划同步时间 |
| `concurrency_limit` | integer | 并发限制（保留字段） |
| `request_interval_ms` | integer | BGM API 请求间隔（毫秒） |
| `updated_at` | datetime | 配置更新时间 |
| `bound_ip_count` | integer | 已绑定 BGM 作品的 IP 数量（只读） |
...
`auto_sync_enabled` 设为 `true` 时，`next_run_at` 会被立即设置为当前时间（下次调度 tick 触发）；设为 `false` 时清除 `next_run_at`。修改 `frequency` 会自动重新计算 `next_run_at`。
...
- **说明**：立即在后台线程中触发一次 BGM 同步任务，遍历所有已绑定 BGM 作品的 IP 并同步角色。
...
```json
{
  ...
  "trigger": "manual",
  "status": "running",
  "started_at": "2025-06-28T12:00:00Z",
  "finished_at": null,
  "triggered_by": "admin",
  "total_ips": 10,
  "success_count": 0,
...
| ------ | ---- | ---- |
| `id` | integer | 任务 ID |
| `trigger` | string | `"manual"`（手动）或 `"scheduled"`（定时） |
| `status` | string | `"running"` / `"succeeded"` / `"partial"` / `"failed"` / `"cancelled"` |
| `started_at` | datetime | 开始时间 |
| `finished_at` | datetime | 结束时间（正在运行时为 `null`） |
| `triggered_by` | string | 触发者的用户名 |
| `total_ips` | integer | 待处理的 IP 数量 |
| `success_count` | integer | 成功数 |
| `failed_count` | integer | 失败数 |
| `skipped_count` | integer | 跳过数 |
| `created_total` | integer | 新建角色数 |
| `linked_total` | integer | 回填 BGM ID 数 |
| `duration_ms` | integer | 持续毫秒数（正在运行时为 `null`） |
| `error_message` | string | 错误信息（无错误时为 `null`） |

**错误**：如果已有正在运行的任务，返回 `409 Conflict`，`{"detail": "已有同步任务在执行中"}`。
...
| `trigger` | string | 按触发方式过滤 |
| `trigger__in` | string | 多触发方式过滤，逗号分隔 |
| `started_at__gte` | date | 开始时间下界 |
| `started_at__lte` | date | 开始时间上界 |
...
{
  "count": 20,
  "next": "http://example.com/api/admin/bgm-sync/jobs/?page=2",
  "previous": null,
  "results": [
    {
      "id": 1,
      ...
      "status": "succeeded",
      "started_at": "2025-06-28T12:00:00Z",
      "finished_at": "2025-06-28T12:05:00Z",
      "triggered_by": "admin",
      "total_ips": 10,
      "success_count": 8,
      "failed_count": 1,
...
| 参数名 | 类型 | 说明 |
| ------ | ---- | ---- |
| `status` | string | 按状态过滤：`success` / `no_change` / `skipped_unbound` / `error` |
| `ip_name_snapshot` | string | 按 IP 名称模糊搜索 |
...
| ------ | ---- | ---- |
| `ip` | integer | IP 作品 ID |
| `ip_name_snapshot` | string | 同步时的 IP 名称快照 |
| `bgm_subject_id` | integer | 绑定的 BGM 作品 ID |
| `status` | string | `"success"` / `"no_change"` / `"skipped_unbound"` / `"error"` |
| `created_count` | integer | 新建角色数 |
| `linked_count` | integer | 回填 BGM ID 数 |
| `subject_type_updated` | boolean | 是否更新了作品类型 |
...
OCR 接口用于识别购物订单截图，自动提取商品名称、价格、数量、入手日期、店铺信息，并匹配合适的 IP、角色、品类等元数据。

> **鉴权说明**：本章节所有接口均需要登录，并在请求头携带 `Authorization: Bearer <access_token>`。  
> 任意登录用户（User / Admin）均可调用。该接口有独立的速率限制（按用户限流）。

### 12.1 识别订单截图

- **URL**：`POST /api/ocr/recognize/`
- **说明**：
  - 上传订单截图图片，后端通过 PaddleOCR 识别文字并进行结构化解析。
  - 自动拆分为多个商品条目（如果截图包含多件商品）。
  - 识别结果会匹配本地数据库中的 IP、角色和品类信息。
  - 识别结果会缓存 5 分钟（相同图片的重复请求直接返回缓存）。
  - `mode=preorder` 时按**预购定金单**解析：整图解析为单个预购（不拆分多商品、不做元数据匹配），返回定金 / 尾款 / 预计补款时间等字段，供「预购与尾款提醒」快速录入。

#### 请求体（multipart/form-data）

| 字段名 | 类型 | 必填 | 说明 |
| ------ | ---- | ---- | ---- |
| `image` | file | 是 | 订单截图，最大 10MB |
| `mode` | string | 否 | `goods`（默认，谷子录入）或 `preorder`（预购定金单） |
| `confidence_threshold` | float | 否 | 元数据匹配置信度阈值，低于此值的结果将被过滤，默认 `0.0`（仅 goods 模式生效） |
...
```json
{
  ...
  "raw_text": "所有回归的\nOCR文字行\n...",
  "suggestions": {
    "ip": { "id": 1, "name": "原神", "confidence": 0.85 },
    "characters": [
      { "id": 42, "name": "荧", "confidence": 0.88 }
    ],
    "category": { "id": 5, "name": "徽章/吧唧", "confidence": 0.76 }
  },
  "items": [
    {
      "name": "原神 荧 吧唧",
      ...
      "shop_name": "官方旗舰店",
      "raw_text": "商品标题行...\n¥12.50\n×1\n...",
      "source_lines": ["商品标题行...", "¥12.50", "×1"],
      "warnings": [],
      "suggestions": {
        "ip": { "id": 1, "name": "原神", "confidence": 0.85 },
        "characters": [ { "id": 42, "name": "荧", "confidence": 0.88 } ],
        "category": { "id": 5, "name": "徽章/吧唧", "confidence": 0.76 }
      }
    }
  ]
...
| 字段名 | 类型 | 说明 |
| ------ | ---- | ---- |
| `name` | string \| null | 手办名称（去括号组与尾部品牌词后） |
| `platform` | string \| null | 平台推断：`淘宝` / `哔哩哔哩会员购`，无法推断为 null |
| `shop_name` | string \| null | 店铺名称 |
| `order_no` | string \| null | 订单号（≥14 位数字串） |
| `deposit_amount` | string \| null | 定金金额（优先"实付价/实付款"，定金单实付即定金） |
| `balance_amount` | string \| null | 尾款金额（未识别为 null） |
| `estimated_month` | string \| null | 预计补款时间 `YYYY-MM`（出货月即补款月；季度粒度存季度首月） |
| `time_granularity` | string | `month`（默认）/ `quarter`（识别到季度文案时） |
| `raw_text` | string | 识别的完整原始文本 |
| `warnings` | array[string] | 识别警告（如尾款/出货时间未识别、多商品截图仅取第一条） |

> 预购定金单判定：全文命中 `定金 / 订金 / 尾款 / 补款 / 预售 / 预定` 任一关键词；非定金单截图在 preorder 模式下返回 400。`全款` 不计入判定（全款预付不是定金+尾款模式，同样返回 400）。
...
| 400 | 图片中未识别到文字 |
| 400 | 无法从截图中解析出商品条目 |
| 400 | preorder 模式下截图非定金单 |
| 500 | OCR 内部处理失败 |

#### 识别说明

1. **图片预处理**：对于高个子手机截图（宽高比 ≥ 1.6，高度 ≥ 1600px），自动裁剪顶部 10% 和底部 1% 的 UI 元素（状态栏、导航栏）；缩放最长边至 1280px。
2. **OCR 引擎**：使用 PaddleOCR（PP-OCRv4，中文模型）。
...
6. **预购解析（preorder 模式）**：以"店铺行之下、汇总行之上"为商品窗口，名称优先取与首个价格同行的非噪音行；定金优先取"实付价/实付款"行；尾款匹配 `尾款N元`；预计补款时间按优先级匹配：① 季度文案（`预计2027年Q2出货`、`预计2027年第四季度` 等，返回 quarter 粒度、存季度首月）→ ② 带年月份（`预计2027年4月出货`，需 `预计` 前缀，出货词可选）→ ③ 出货/补款/尾款字样 + `时间` 后的带年日期（`出货时间：2027年4月`、`尾款时间：2027-04-15`，需带年份，"间"字可省略；纯日期文案如 `尾款：2027年4月15日` 不作为预计补款月）→ ④ 仅月份（`预计12月到货`，按识别当天推断年份）；多个不同实付价时警告并仅按第一条登记。
...
社团商品在草稿状态导入时，限时成就以商品状态首次进入 `in_cabinet`、`outdoor` 或 `sold` 的时间作为生效时间；时间窗结束后再变为有效不会追溯解锁。
...
奖励停用只影响库存中尚未拥有该奖励的用户和新的目录展示。已拥有但停用的奖励仍会保留在用户奖励库中，可以继续领取、装备和公开展示。
