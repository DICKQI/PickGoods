...
| `confidence_threshold` | float | 否 | 元数据匹配置信度阈值，低于此值的结果将被过滤，默认 `0.0` |
...
| 字段名 | 类型 | 说明 |
| ------ | ---- | ---- |
| `name` | string | 识别到的商品名称（取 `items[0]`） |
| `price` | string | 识别到的单价（取 `items[0]`） |
| `quantity` | int | 识别到的数量（取 `items[0]`） |
| `purchase_date` | string | 识别到的入手日期 `YYYY-MM-DD` |
| `is_official` | boolean | 识别是否为官谷 |
| `shop_name` | string | 店铺名称 |
| `raw_text` | string | 识别的完整原始文本 |
| `suggestions` | object | 元数据匹配建议：IP、角色、品类及置信度 |
| `items` | array[object] | 拆分后的商品条目列表 |
| `items[].source_lines` | array[string] | 该商品对应的原始 OCR 行 |
| `items[].warnings` | array[string] | 识别警告信息 |
