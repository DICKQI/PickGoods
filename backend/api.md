...
> 预购定金单判定：全文命中 `定金 / 订金 / 尾款 / 补款 / 预售 / 预定 / 全款` 任一关键词；非定金单截图在 preorder 模式下返回 400。
...
6. **预购解析（preorder 模式）**：以"店铺行之下、汇总行之上"为商品窗口，名称优先取与首个价格同行的非噪音行；定金优先取"实付价/实付款"行；尾款匹配 `尾款N元`；预计补款时间匹配 `预计YYYY年M月出货`（带年）→ `预计M月出货`（推断年份）→ 季度文案（`预计2027年Q2出货` 等，返回 quarter 粒度）；多个不同实付价时警告并仅按第一条登记。
