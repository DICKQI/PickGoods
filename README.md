...
> **截图说明：** 以下界面来自生产环境和本地社团测试数据；截图用于功能展示，部分数据为测试数据，涉及个人账号、订单号和联系方式等敏感信息的区域已做遮挡处理。
...
按关键词、IP、角色、品类、主题、状态、位置和官谷属性组合检索。卡片列表支持标准视图与相似随机浏览；打开详情后可查看主图、附加图片、购买信息、收纳位置、备注和同主题收藏。
